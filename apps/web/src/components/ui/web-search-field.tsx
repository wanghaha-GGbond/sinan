...
export function WebSearchField({
  ...
  return (
    <div className="flex items-center gap-2 rounded-xl border border-border bg-card p-2 transition-colors focus-within:border-primary-surface-border">
      ...
      <Input aria-label="公司名称" value={value} onChange={(event) => onChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onSubmit?.() }} placeholder={placeholder} className="h-10 border-0 bg-transparent px-1 text-foreground shadow-none focus-visible:ring-0" />
