    ...
    <SolidCard variant="elevated" className="mb-4 p-5" data-testid="company-intelligence-panel">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <h2 className="mt-3 text-xl font-semibold text-[#111827]">像看公司说明书一样看清这家公司</h2>
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-5">
        {cards.map((card) => (
          <Link
            key={card.label}
            href={card.href}
            className="rounded-[24px] border border-[#E5E7DB]/70 bg-[#F8FAF4] p-4 transition hover:-translate-y-0.5 hover:bg-white"
          >
            <div className="flex items-center gap-2 text-sm font-semibold text-[#111827]">
              <card.icon className="size-4 text-[#19C37D]" />
              {card.label}
            </div>
