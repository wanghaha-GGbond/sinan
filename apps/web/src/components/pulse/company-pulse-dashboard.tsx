...
export function CompanyPulseDashboard() {
  ...
  async function shareWeeklyPulse() {
    const text = "我的 Company Pulse 周报：本周工作 47.5 小时，平均 20:36 下班，恢复度 72，工作平衡 64。"
    try {
      if (navigator.share) {
    ...
    <div className="space-y-7">
      <SolidCard variant="elevated" className="overflow-hidden p-0" data-testid="pulse-share-card">
        <div className="flex flex-col gap-3 border-b border-border bg-foreground px-5 py-4 text-white sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-semibold text-white/65">COMPANY PULSE · 8月3日 - 8月9日</p>
            <h2 className="mt-1 text-xl font-semibold">我的工作周报</h2>
          </div>
        ...
        <section className="border-y border-border py-6" aria-labelledby="company-pulse-title">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h2 id="company-pulse-title" className="mt-1 text-xl font-semibold text-foreground">北辰智造 Company Pulse</h2>
              <p className="mt-1 text-sm text-muted-foreground">像行情一样观察团队状态，不显示任何个人轨迹。</p>
            </div>
