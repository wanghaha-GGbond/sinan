 ...
 *
 * 关键不变量:
 *   - dm_threads: (LEAST(A,B), GREATEST(A,B)) 唯一, 一对用户一个 thread
...
export async function createThreadOrRequest(
  ...
  if (!needsRequest) {
    ...
    const [existing] = await db
      ...
      .limit(1)
    if (existing) return { kind: "thread", threadId: existing.id, alreadyExisted: true }
...
export type RequestMutationResult =
  ...
  | { ok: false; reason: "request_not_found" | "not_recipient" | "not_pending" | "request_not_for_actor" }
...
export type RejectResult =
  ...
  | { ok: false; reason: "request_not_found" | "not_recipient" | "not_pending" }
...
export type WithdrawResult =
  | { ok: true; requestId: string }
  | { ok: false; reason: "request_not_found" | "not_sender" | "not_pending" }
...
export async function listMyThreads(userId: string): Promise<ThreadListItem[]> {
  ...
  const otherIds = Array.from(
    new Set(
      rows.map((r) => (r.aId === userId ? r.bId : r.aId))
    )
  )
...
export async function listMyRequests(
  ...
  const { db } = await import("@/db/client")
  const whereParts = [or(eq(dmRequests.toUserId, viewerUserId), eq(dmRequests.fromUserId, viewerUserId))]
