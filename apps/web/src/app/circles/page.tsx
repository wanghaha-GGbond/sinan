    ...
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-4 py-8 sm:px-6">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm leading-6 text-muted-foreground">
          入圈需要 1 名现有成员背书, 段位门槛 L{minTrustLevelHint(circles)}。
          圈层是声誉场, 不是社交场。
        </p>
      </header>

      {error && (
        <SolidCard variant="flat" className="p-4 text-sm text-muted-foreground">
          {error}
        </SolidCard>
      )}

      {!circles && !error && (
        <SolidCard variant="flat" className="h-32 animate-pulse" />
      )}

      {circles && circles.length === 0 && (
        <SolidCard variant="flat" className="p-6 text-center text-sm text-muted-foreground">
          圈层即将开放, 运营在筹备首批名单。
        </SolidCard>
      )}
...
function minTrustLevelHint(circles: CircleItem[] | null): string {
  if (!circles || circles.length === 0) return "1"
  const min = Math.min(...circles.map((c) => c.minTrustLevel))
  return String(min)
}
