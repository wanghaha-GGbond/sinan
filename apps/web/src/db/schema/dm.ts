...
export const dmThreads = pgTable(
  ...
  (table) => [
    // 保证两个用户之间只有一个 thread (无论谁先发起)
    // Drizzle 的 uniqueIndex.on() 只接受列, 表达式用 raw 索引。
    // 通过外加的方式: 不在 (table) 数组里, 由迁移显式补一个 unique index.
    // 这里先建一个普通的 participant_a+b 索引(查列表用), 配对唯一由 service 层保证。
