...
在独立 Neon staging 分支按 `apps/web/src/db/migrations/` 事实源验证全部当前迁移、应用启动、核心写入和恢复能力。严禁首次演练直接连接生产分支。
...
2026-07-05 本地 PostgreSQL 16 基线演练已完成：首次演练发现并修复重复索引名及缺失拍卖表迁移；当时的 17 个迁移可从空库应用，第二次执行幂等成功，生成 29 张业务表并记录 17 条迁移日志。当前迁移范围不再沿用这条历史数字，必须以 `apps/web/src/db/migrations/` 目录为事实源（当前最新序号为 `0020`），并由 CI 每次从空库重跑和重复执行。备份、删除、重建和哨兵恢复成功；数据库并发测试 5/5、核心 HTTP 发布闭环 1/1 通过。该结果不替代 Neon staging 分支切换、DirectMail 和生产监控演练。
